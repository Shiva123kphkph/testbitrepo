...
സിനിമ ലഭിക്കണം എങ്കിൽ താങ്കൾ ഗുഗിൾ നോക്കി സിനിമയുടെ correct spelling ഇവിടെ send ചെയ്യുക എങ്കിലേ താങ്കൾ ഉദ്ദേശിക്കുന്ന സിനിമ എനിക്ക് അയച്ചു തരാൻ കഴിയുകയുളളു 😀""",show_alert=True)
        ...
        buttons= [[
            ...
            InlineKeyboardButton('♥️ Source', url='https://t.me/bheeshmaparvam')
            ],[
            InlineKeyboardButton('🏠 Home', callback_data='start'),
...
<i>Please Go to Google and Confirm the Correct Spelling 🥺🙏</i></b>""",
        ...
        if imdb and imdb.get('poster'):
            await message.reply_photo(photo=imdb.get('poster'), caption=f"<b>🕵️‍♂️ REQUESTED : {search} \n‌‌‌‌\n🏷 Title: <a href={imdb['url']}>{imdb.get('title')}</a>\n🎭 Genres: {imdb.get('genres')}\n📆 Year: <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a>\n🌟 Rating: <a href={imdb['url']}/ratings>{imdb.get('rating')}</a> / 10\n🎙️ Languages : <code>{imdb.get('languages')} </code>\n👨‍🎤ACTORS : {imdb.get('actors')} \n🌏COUNTRY : {imdb.get('country')} \n🖋 StoryLine: <code>{imdb.get('plot')} </code></b>", reply_markup=InlineKeyboardMarkup(btn))
        elif imdb:
            await message.reply_text(f"<b>🕵️‍♂️ REQUESTED : {search} \n‌‌‌‌\n🏷 Title: <a href={imdb['url']}>{imdb.get('title')}</a>\n🎭 Genres: {imdb.get('genres')}\n📆 Year: <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a>\n🌟 Rating: <a href={imdb['url']}/ratings>{imdb.get('rating')}</a> / 10\n🎙️ Languages : <code>{imdb.get('languages')} </code>\n👨‍🎤ACTORS : {imdb.get('actors')} \n🌏COUNTRY : {imdb.get('country')} \n🖋 StoryLine: <code>{imdb.get('plot')} </code></b>", reply_markup=InlineKeyboardMarkup(btn))
        else:
